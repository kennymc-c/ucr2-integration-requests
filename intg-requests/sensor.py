...
async def add_tcp_text_sensor(ent_id: str, name: str):
    """Function to add a tcp text response sensor entity"""
    ...
    _LOG.info("Added tcp text response sensor entity with id " + ent_id + " and name " + str(name))



def update_tcp_text_sensor(entity_id: str, response: str):
    """Parse tcp text response with configured regular expression and update sensor entity value"""
    ...
    _LOG.info("Updated tcp text response sensor value to " + response)
