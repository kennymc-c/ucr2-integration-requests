...
- Updated Requests Python module to 2.34.0
...
- Fixed crash after restart if title case option for custom select entity options was enabled
...
- Custom Entities: Fixed non working command sequence command
...
- Custom Entities: Fixed a bug with integer variables

### Added

- Added a check if the source, command or command sequence parameter is empty and return a bad request error

### Changed

- Updated pyyaml to 6.0.3
...
- Custom Entities: Fixed resource not found error with multiple entities in a configuration

### Changed

- Custom Entities: Only update power state if the command was successful

## [0.8.1] - 2025-08-22

### Fixed

- Custom Entities: Fixed issues with send command and send command sequence
